...
public class UItests : PageTest {
	...
	public async Task logIn() {
		...
		await Page.GetByPlaceholder("name@example.com").FillAsync("newtest@test.dk");
		...
		await Expect(Page.GetByRole(AriaRole.Heading, new() { Name = "Tell others what's on your" }))
			.ToBeVisibleAsync();
		await Expect(Page.GetByText("Tell others what's on your mind newtestuser! Cheep")).ToBeVisibleAsync();
	}
	...
	public async Task registerEmailInUse() {
		...
		await Page.GetByPlaceholder("name", new() { Exact = true }).FillAsync("testing1234");
	...
	public async Task logInLogOut() {
		...
		await Page.GetByPlaceholder("password").FillAsync("Test1234!");
		await Page.GetByRole(AriaRole.Button, new() { Name = "Log in" }).ClickAsync();
		await Page.GetByRole(AriaRole.Link, new() { Name = "Logout [newtestuser]" }).ClickAsync();
